...
export function GamePlayLanscape({
  currentRoom,
  currentSession,
  gamerId,
  otherPlayers,
  ...
  const [activePanel, setActivePanel] = useState<"myMelds" | "tableMelds" | "stats" | null>(null);
      ...
      <header className="flex items-center justify-between px-10 py-6">
        ...
        <div className="flex items-center gap-6">
          <div className="text-right">
            <p className="text-sm text-gray-600 dark:text-gray-400">รอบที่ {currentSession.roundNumber}</p>
          ...
          <div className="relative h-20 w-20">
            <svg viewBox="0 0 36 36" className="h-full w-full -rotate-90">
              ...
              />
              <path
                className={remainingSeconds <= 10 ? "text-red-500" : "text-emerald-500"}
                stroke="currentColor"
                strokeWidth="3"
                fill="transparent"
                strokeLinecap="round"
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className={`text-lg font-bold ${remainingSeconds <= 10 ? "text-red-600" : "text-emerald-600"}`}>
                {formattedRemaining}
              </span>
              <span className="text-[10px] uppercase text-gray-500 dark:text-gray-400">/ {turnTimeLimit}s</span>
            </div>
          </div>
        </div>
      ...
      {isGameFinished && (
        <div className="absolute left-1/2 top-24 z-30 flex w-[520px] -translate-x-1/2 items-center gap-4 rounded-3xl border border-emerald-300 bg-emerald-50/90 px-6 py-4 shadow-xl dark:border-emerald-700 dark:bg-emerald-900/30">
          ...
          <div>
            ...
            {winningTypeLabel && (
              <p className="text-sm text-emerald-600/80 dark:text-emerald-200/80">วิธีชนะ: {winningTypeLabel}</p>
            )}
          </div>
        </div>
      )}
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span className="max-w-[120px] truncate">{player.displayName || "ผู้เล่น"}</span>
                  </div>
                  ...
                  <div className="mt-2 flex items-center gap-1 rounded-xl bg-white/80 px-3 py-1 text-xs text-gray-700 shadow dark:bg-slate-800/80 dark:text-gray-200">
                    <div className="flex items-center gap-0.5">
                      ...
                      {remainingCards > 0 && (
                        <span className="ml-1 text-[10px] font-semibold text-gray-500 dark:text-gray-300">+{remainingCards}</span>
                      )}
                    </div>
            ...
            <div className="relative flex w-[520px] flex-col items-center gap-6 rounded-3xl border border-white/70 bg-white/80 px-8 py-6 text-gray-800 shadow-lg dark:border-slate-800 dark:bg-slate-900/80 dark:text-gray-200">
              <div className="flex items-center gap-8">
                ...
                <div className="flex flex-col items-center">
                  <div className={`text-3xl font-bold ${remainingSeconds <= 10 ? "text-red-600" : "text-emerald-600"}`}>
                    {formattedRemaining}
                  </div>
                  <p className="text-sm text-gray-600 dark:text-gray-300">เวลาต่อเทิร์น</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{turnTimeLimit} วินาที</p>
                </div>
              ...
              <div className="min-h-[32px] text-center text-sm text-blue-900 dark:text-blue-200">
                {guidanceMessage || (isMyTurn ? "เลือกการกระทำในเทิร์นของคุณ" : "รอผู้เล่นคนอื่นจัดการเทิร์น")}
              </div>
            </div>
            ...
            <div className="h-[calc(80vh-120px)] overflow-y-auto pr-2">
              {activePanel === "myMelds" && (
                <div className="space-y-4">
                  {myMelds.length === 0 ? (
                    <p className="text-sm text-gray-500 dark:text-gray-400">ยังไม่มีไพ่ที่คุณเกิดในรอบนี้</p>
                  ) : (
              ...
              {activePanel === "tableMelds" && (
                <div className="space-y-4">
                  {communityMelds.length === 0 ? (
                    <p className="text-sm text-gray-500 dark:text-gray-400">ยังไม่มีผู้เล่นอื่นเกิดไพ่บนโต๊ะ</p>
                  ) : (
              ...
              {activePanel === "stats" && (
                <div className="space-y-4 text-sm text-gray-600 dark:text-gray-300">
                  <div className="rounded-2xl border border-gray-200 bg-white/80 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/80">
                    <h4 className="text-base font-semibold text-gray-700 dark:text-gray-200">สถานะเกม</h4>
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-white/80 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/80">
                    <h4 className="text-base font-semibold text-gray-700 dark:text-gray-200">ผู้เล่นในห้อง</h4>
                    <div className="mt-3 space-y-2">
